...
#[derive(Clone)]
pub struct BackblazeProvider {
    account_id: String,
    application_key: String,
}

impl BackblazeProvider {
    pub fn new(account_id: &str, application_key: &str) -> Self {
        Self { account_id: account_id.to_string(), application_key: application_key.to_string() }
    }
}

impl StorageProvider for BackblazeProvider {
    fn upload_blocking(&self, bucket: &str, file_path: &str) -> Result<(), Box<dyn Error>> {
        upload_to_backblaze_blocking(&self.account_id, &self.application_key, bucket, file_path)
    }

    fn download_blocking(&self, bucket: &str, file_name: &str, dest: &Path) -> Result<(), Box<dyn Error>> {
        download_from_backblaze_blocking(&self.account_id, &self.application_key, bucket, file_name, dest)
    }

    fn show_history_blocking(&self, bucket: &str, retention: Option<Duration>) -> Result<(), Box<dyn Error>> {
        show_remote_history_blocking(&self.account_id, &self.application_key, bucket, retention)
    }
}
